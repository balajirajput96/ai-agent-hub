...
export default function ChatDashboard() {
  ...
  const handleCreateSession = (e: React.FormEvent) => {
    e.preventDefault();
    const title = newSessionTitle.trim() || `Agent Session ${new Date().toLocaleTimeString()}`;
  ...
  const handleFileSelection = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const supportedTypes = new Set(["application/pdf", "text/plain", "text/markdown", "text/csv", "application/json"]);
    if (!supportedTypes.has(file.type)) {
      setUploadError("Choose a PDF, TXT, MD, CSV, or JSON document.");
      return;
    }
    if (!file.size || file.size > 8 * 1024 * 1024) {
      setUploadError(file.size ? "Documents must be 8 MB or smaller." : "The selected document is empty.");
      return;
    }
    setUploadError("");
    const reader = new FileReader();
    reader.onload = () => uploadAttachmentMutation.mutate({
      sessionId: activeSessionId,
      fileName: file.name,
      fileBase64: String(reader.result),
      mimeType: file.type,
    });
    reader.onerror = () => setUploadError("The document could not be read. Please try another file.");
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if ((!inputMessage.trim() && !selectedAttachment) || !activeSessionId) return;
      ...
      <aside className={`${mobilePanel === "sessions" ? "flex" : "hidden"} h-[100dvh] w-full flex-col border-r border-slate-800 bg-slate-900/90 md:flex md:h-screen md:w-80 md:shrink-0`}>
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-base tracking-tight">AI Agent Hub</h1>
        ...
        <div className="p-4 border-b border-slate-800 bg-slate-950/40">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs bg-slate-900 p-2 rounded-lg border border-slate-800">
              ...
              <Badge variant="outline" className={healthData?.github.status === "connected" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20"}>
            ...
            <div className="flex items-center justify-between text-xs bg-slate-900 p-2 rounded-lg border border-slate-800">
              ...
              <Badge variant="outline" className={healthData?.huggingface.status === "connected" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20"}>
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-1.5">
            <div className="text-xs font-semibold text-slate-500 px-2 mb-1">Recent Sessions</div>
        ...
        <div className="p-4 border-t border-slate-800 flex items-center gap-3 bg-slate-950/60">
          ...
          <div className="truncate">
            <div className="text-xs font-semibold text-white truncate">{user.name || "Authenticated User"}</div>
            <div className="text-[10px] text-slate-400 truncate">{user.email || "Manus User"}</div>
          </div>
        </div>
      </aside>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            <ScrollArea className="flex-1 p-6 space-y-6">
              {!activeSessionId ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 space-y-3">
                  ...
                  <p className="text-sm">Select or create a chat session from the sidebar to begin.</p>
                </div>
              ) : messagesQuery.data?.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-500 space-y-3">
                  <Sparkles className="w-10 h-10 text-indigo-500/40" />
                  <h3 className="font-medium text-slate-300">How can I help you today?</h3>
                  <p className="text-xs max-w-sm">
                    Try asking: &quot;Search GitHub for React components&quot; or attach a document for analysis.
                  </p>
                </div>
              ) : (
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-900/40">
              <form onSubmit={handleSendMessage} className="max-w-3xl mx-auto">
                ...
                <div className="flex gap-3">
                  ...
                  <Button type="submit" disabled={!activeSessionId || (!inputMessage.trim() && !selectedAttachment) || sendMessageMutation.isPending || uploadAttachmentMutation.isPending} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-6 shadow-lg shadow-indigo-600/25"><Send className="w-4 h-4" /></Button>
                </div>
              </form>
            </div>
          </div>
          ...
          <div className={`${mobilePanel === "logs" ? "flex" : "hidden"} h-full w-full flex-col border-t border-slate-800 bg-slate-900/60 lg:flex lg:h-full lg:w-80 lg:border-l lg:border-t-0`}>
            ...
            <ScrollArea className="flex-1 p-4">
              <div className="mb-5">
                <h4 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Private documents</h4>
                {attachmentsQuery.data?.length ? (
                  <div className="space-y-2">
                    {attachmentsQuery.data.map((attachment) => (
                      <a key={attachment.id} href={attachment.fileUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-950 p-2 text-xs text-slate-300 hover:border-indigo-500/40">
              ...
              {!activeSessionId ? (
                <div className="text-xs text-slate-500 text-center py-8">No active session</div>
              ) : toolLogsQuery.data?.length === 0 ? (
                <div className="text-xs text-slate-500 text-center py-8">
                  No tool calls executed in this session yet. Try asking the agent to search GitHub.
                </div>
              ) : (
                <div className="space-y-3">
                  {toolLogsQuery.data?.map((log) => (
                    <div key={log.id} className="bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-indigo-300">{log.toolName}</span>
                        <Badge variant="outline" className={log.status === "success" ? "text-emerald-400 border-emerald-500/20 bg-emerald-500/10" : "text-amber-400 border-amber-500/20 bg-amber-500/10"}>
                          {log.status}
                        </Badge>
                      </div>
                      ...
                      {log.outputResult && (
                        <details className="text-[11px] text-slate-300">
                          <summary className="cursor-pointer text-indigo-400 font-medium">View Output</summary>

...
describe("document validation", () => {
  ...
  it("rejects mismatched, malformed, and empty document payloads", () => {
    ...
    expect(() =>
      decodeDocumentPayload("data:text/plain;base64,not valid!", "text/plain")
    ).toThrow("document payload is invalid");
    expect(() =>
      decodeDocumentPayload("data:text/plain;base64,", "text/plain")
    ).toThrow("selected document is empty");
  });
});
